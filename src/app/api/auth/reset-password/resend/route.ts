...
/**
 * パスワードリセットコード再送エンドポイント。
 * reset_verify_token の HMAC を検証し、新しいコードを発行してメールを再送する。
 * 成功時は新しい tokenId を含む reset_verify_token クッキーを発行する。
 */
export async function POST(request: Request) {
  return withApiHandler(async () => {
    ...
    const result = await resendResetCode(userRepo, tokenRepo, emailService, { tokenId, emailHash });
    ...
    response.cookies.set("reset_verify_token", newCookieValue, {
      httpOnly: true,
