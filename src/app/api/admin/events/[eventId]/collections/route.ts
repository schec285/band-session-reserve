...
/**
 * 参加費徴収状況の更新エンドポイント。
 * admin のみアクセス可。collected: true で徴収済み記録、false で取り消し。
 */
export async function PATCH(
  ...
  return withApiHandler(async () => {
    ...
    const parsed = SetCollectionSchema.safeParse(body);
    ...
    const result = await setCollection(repo, eventId, parsed.data.userId, parsed.data.collected);
